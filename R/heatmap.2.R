...
## APG: Added two mpg fields to control for the distance between the axis title, labels and line, respectively
margins = c(5, 5),
ColSideColors,
RowSideColors,
...
cex.lab = 1,
mgps=c(margins[1] - 1.25 + cex.lab / 5,1,0),  # APG self scaling with margin and cex
xmgp=mgps[1], # APG By default is the first argument of mgps, if used will overwrite mgps[1] either for x label
ymgp=mgps[1], # or for y label
labRow = NULL,
labCol = NULL,
srtRow = NULL,
srtCol = NULL,
...
# to be able to change the x and y axes size with cex, and additional ones to control for some margins
...
if(!is.numeric(mgps) || length(mgps) != 3) # APG added
stop("`mpgs' must be a numeric vector of length 3")
if(!is.numeric(xmgp)) # APG added
stop("`xmpg' must be numeric")
if(!is.numeric(ymgp)) # APG added
stop("`ympg' must be numeric")
...
par(mar = c(margins[1],0, 0,0.5),mgp=mgps) # Added mgp, not working
image(rbind(1:nr), col = RowSideColors[rowInd], axes = FALSE)
plot.index <- plot.index + 1
}
if(!missing(ColSideColors)) {
par(mar = c(0.5,0, 0,margins[2]),mgp=mgps)# Added mgp, not working
image(cbind(1:nc), col = ColSideColors[colInd], axes = FALSE)
plot.index <- plot.index + 1
}
## draw the main carpet
par(mar = c(margins[1], 0, 0, margins[2]),mgp=mgps)# Added mgp, not working
#if(scale != "none" || !symm)
#  {
x <- t(x)
cellnote <- t(cellnote)
...
# APG I modify the following code to be able to modify the x and y labels with cex.lab
...
#   
    ...
    line = xmgp[1],#margins[1] - 1.25 + cex.lab / 5,
    cex = cex.lab)
    if(!is.null(ylab)) mtext(ylab, side = 4,
    line = ymgp[1],#margins[2] - 1.25 + cex.lab / 5,
    cex = cex.lab)
